...
def getPagesDict(word,page,search_dict):
    resultDict={}
    '''
    if search_dict.get('huajiao') == 'true':
        huajiaoUrl = 'http://www.huajiao.com/search?q='+ word
        baiduUrl = 'https://www.baidu.com/s?wd=' + word + '&pn=' + str(10 * (page - 1))
        resultDict['huajiao'] = huajiaoUrl
    if search_dict.get('baidu')=='true':
        baiduUrl='https://www.baidu.com/s?wd='+word+'&pn='+str(10*(page-1))
        resultDict['baidu']=baiduUrl
    if search_dict.get('china_so')=='true':
    ...
    if search_dict.get('huajiao') == 'true':
        huajiaoUrl = 'http://www.huajiao.com/search?q=' + word
        resultDict['huajiao'] = huajiaoUrl
    if search_dict.get('douyu') == 'true':
        douyuUrl = 'https://www.douyu.com/search/'+ word
        resultDict['douyu'] = douyuUrl
    if search_dict.get('meipai') == 'true':
        meipaiUrl = 'https://www.meipai.com/search/all?q='+ word +'&page='+str(page)+'&fromAll=1'
        resultDict['meipai'] = meipaiUrl
    if search_dict.get('zhanqi') == 'true':
        zhanqiUrl = 'http://www.zhanqi.tv/search?q='+ word
        resultDict['zhanqi'] = zhanqiUrl
    if search_dict.get('bili') == 'true':
        biliUrl = 'http://live.bilibili.com/search/index/?keyword=' + word
        resultDict['bili'] = biliUrl
    if search_dict.get('huya') == 'true':
        huyaUrl = 'http://www.huya.com/search.php?hsk=' + word
        resultDict['huya'] = huyaUrl
    if search_dict.get('yy') == 'true':
        yyUrl = 'http://www.yy.com/search?q=' + word
        resultDict['yy'] = yyUrl
