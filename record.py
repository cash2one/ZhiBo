...
def onQueryRecordLog(ip,content_type,key,suggestion_map,relate_search_map,hint_top_map):
	...
	try:
		...
		#print cur_time
		exe = "INSERT INTO " + table_name + " (time,ip,type,keyword,suggestion,relate_search,hint_top) VALUES ('%s','%s','%s','%s','%s','%s','%s')" %(cur_time,ip,type,keyword,suggestion_map,relate_search_map,hint_top_map)
		conn.execute(exe)
	finally:
		conn.close()
